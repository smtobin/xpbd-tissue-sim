...
void RefinedTetMesh::_removeVertex(int vertex_index)
{
    _vertices.erase(vertex_index);
    _latest_removed_vertices.emplace_back(vertex_index, -1, -1);

    int hanging_vert_removed = _hanging_vertices.erase(vertex_index);
    if (hanging_vert_removed)
    {
        // std::cout << " _updateVertexElementMapForRemovedElement remove vertex " << elem_to_remove[k] << " from hanging vertices!" << std::endl;
        _latest_removed_hanging_vertices.push_back(vertex_index);
    }
    
    // vertex no longer in mesh, so clear its adjacent vertices list
    _vertex_adjacent_vertices[vertex_index].clear();
}

int RefinedTetMesh::_addFace(const Vec3i& new_face, int elem_with_face)
{
    // same as before, but now we append the new face index to the list of newly added faces
    int new_face_index = TetMesh::_addFace(new_face, elem_with_face);
    _latest_new_faces.push_back(new_face_index);
    
    // if there is a FaceNode already associated with this face, mark it as on the surface
    if (auto it = _face_to_face_node_map.find(Face(new_face[0], new_face[1], new_face[2])); it != _face_to_face_node_map.end())
    {
        int face_node_index = it->second;
...
TetMesh::RemovedElement RefinedTetMesh::removeElement(int elem_index)
{
    ...
    if (auto search = _element_to_tree_node_map.find(elem_index); search != _element_to_tree_node_map.end())
    {
        ...
        ElementTreeNode& node_to_remove = _element_tree_nodes[node_index];
        ...
        _removeFeaturesForRemovedElementTreeNode(node_to_remove);
        _updateDescendantFeatureHierarchyForRemovedElement(node_to_remove);
    ...
    {
        // create a temporary ElementTreeNode that has the necessary information
        ElementTreeNode temp_tree_node(elem_to_remove, ElementTreeNode::INVALID_INDEX, 0);
        // edge nodes
        ...
        for (int i = 0; i < 4; i++)
        {
            for (int j = i+1; j < 4; j++)
            {
                if (auto it = _edge_to_edge_node_map.find(Edge(elem_to_remove[i], elem_to_remove[j])); it != _edge_to_edge_node_map.end())
                {
                    temp_tree_node.edge_nodes[num_edge_nodes] = it->second;
                    num_edge_nodes++;
                }
            }
        }
        ...
        for (int i = 0; i < 4; i++)
        {
            for (int j = i+1; j < 4; j++)
            {
                for (int k = j+1; k < 4; k++)
                {
                    if (auto it = _face_to_face_node_map.find(Face(elem_to_remove[i], elem_to_remove[j], elem_to_remove[k])); it != _face_to_face_node_map.end())
                    {
                        temp_tree_node.face_nodes[num_face_nodes] = it->second;
                        num_face_nodes++;
                    }
                }
            }
...
void RefinedTetMesh::_updateDescendantFeatureHierarchyForRemovedElement(ElementTreeNode& node)
{
    ...
    // iterate through the root's edge nodes and look for edges whose parent feature has
    ...
    while (!face_nodes_to_update.empty())
    {
        ...
        else
        {
            ...
            int new_face_index = _addFace(Vec3i(face_node.face.index1, face_node.face.index2, face_node.face.index3), element_index_with_face);
    ...
    while (!edge_nodes_to_update.empty())
    {
        ...
        if (!edge_node.is_leaf && d > 0)
        {
            ...
            if (edge_node.child_vertex != ElementTreeNode::INVALID_INDEX)
            {
                ...
                if (hanging_vert_removed)
                {
                    // std::cout << "   _updateFeatureHierarchyForRemovedElementEdgeAndFaceNodes Removed vertex " << edge_node.child_vertex << " from hanging vertices!" << std::endl;
...
void RefinedTetMesh::_updateVertexElementMapForRemovedElement(int element_index)
{
    ...
    for (int k = 0; k < 4; k++)
    {
        ...
        if (vk_map.size() == 0)
        {
            _removeVertex(elem_to_remove[k]);
        }
    }
}
...
int RefinedTetMesh::_addNewElementFromElementTreeNode(int tree_node_index)
{
    ...
    // the edge nodes and face node are in the mesh
    /** TODO: need to edit "in_mesh" of children? */
    // for (const auto& edge_node_index : node.edge_nodes)
    // {
    //     if (edge_node_index == ElementTreeNode::INVALID_INDEX)
    //         continue;
        
    //     _edge_nodes[edge_node_index].in_mesh = true;
    // }
    // for (const auto& face_node_index : node.face_nodes)
    // {
    //     if (face_node_index == ElementTreeNode::INVALID_INDEX)
    ...
    //     _face_nodes[face_node_index].in_mesh = true;
    // }
...
int RefinedTetMesh::_addNewElement(const Vec4i& new_element, bool f012_on_surface, bool f013_on_surface, bool f023_on_surface, bool f123_on_surface)
{
    ...
    if (f012_on_surface)
    {
        ...
        _addFace(new_face, new_elem_index);
    }
    // F013
    if (f013_on_surface)
    {
...
void RefinedTetMesh::_addHangingVerticesForNonLeafEdgeNodes(const ElementTreeNode& root_node, int rel_coarsening_level)
{
    // store the edges and faces that need to be updated 
    // (first int = node index, second int = depth)
    std::stack<std::pair<int,int>> face_nodes_to_update;
    ...
    {
        if (face_node_index != ElementTreeNode::INVALID_INDEX)
            face_nodes_to_update.push({face_node_index, rel_coarsening_level+1});
    }
    ...
    {
        if (edge_node_index != ElementTreeNode::INVALID_INDEX)
            edge_nodes_to_update.push({edge_node_index, rel_coarsening_level+1});
    }
    ...
    while (!edge_nodes_to_update.empty())
    {
        ...
        if (!edge_node.is_leaf && d > 0)
        {
            ...
            if (edge_node.child_vertex != ElementTreeNode::INVALID_INDEX)
            {
                ...
                if (success)
                {
                    // std::cout << "Added vertex " << edge_node.child_vertex << " to hanging vertices!" << std::endl;
...
{
    // std::cout << "\n=== Coarsening element " << element_index << ": " << element(element_index).transpose() << std::endl;
    // clear the latest added vertices and elements
    if (clear_latest)
    {
        _latest_new_vertices.clear();
    ...
    // std::cout << "Root node vertices: " << root_node.vertices.transpose() << std::endl;
        ...
        else 
        {
            ...
            else
            {
                ...
                for (int k = 0; k < 4; k++)
                {
                    ...
                    if (vk_map.size() == 0)
                    {
                        _removeVertex(node.vertices[k]);
                    }
                }
                
            }

            // _updateFeatureHierarchyForRemovedElementTreeNode(node);
    ...
    // starting at the root node, traverse down the feature tree
    // any non-leaf edge nodes have hanging vertices at their midpoint
    _addHangingVerticesForNonLeafEdgeNodes(root_node, rel_coarsening_level);
...
std::tuple<std::vector<int>, std::vector<Vec3i>, std::vector<int>> RefinedTetMesh::submeshForElementClass(int element_class) const
{
    ...
    for (const auto& elem_index : _elements.validIndices())
    {
        if (class_eprop.get(elem_index) == element_class)
        {
            ...
            auto check_face = [&] (const Face& query_face)
            {
                ...
                // we didn't find any adjacent elements
                if (adj_elem_index == -1)
                {
                    // see if there is a face node for this face (i.e. is this face part of refinement?)
                    auto it = _face_to_face_node_map.find(query_face);
                    if (it != _face_to_face_node_map.end())
                    {
                        // there is an associated face node
                        // check if the face node is on the surface
                        if (_face_nodes[it->second].on_surface)
                        {
                            class_faces.insert(query_face);
                        }
                        else
                        {
                            // no elements share this face, not on the surface:
                            // now we must traverse the tree and see if adjacent elements share part of this face
                            // go up the tree first
                            int cur_index = _face_nodes[it->second].parent_face_node;
                            while (cur_index != ElementTreeNode::INVALID_INDEX)
                            {
                                // check the face -> element map for a hit
                                auto range = _face_to_elements_map.equal_range(_face_nodes[cur_index].face);
                                if (range.first != range.second)
                                {
                                    // hit
                                    int found_elem_index = range.first->second;
                                    if (class_eprop.get(found_elem_index) != element_class)
                                    {
                                        class_faces.insert(query_face);
                                        return;
                                    }
                                }
                                cur_index = _face_nodes[cur_index].parent_face_node;
                            }

                            // nothing found going up the tree - traverse towards children now
                            // go all the way to the leaf face node - this should have an element associated with it
                            int leaf_index = it->second;
                            while(!_face_nodes[leaf_index].is_leaf)
                            {
                                for (const auto& child : _face_nodes[leaf_index].child_face_nodes)
                                {
                                    if (child != ElementTreeNode::INVALID_INDEX)
                                    {
                                        leaf_index = child;
                                        break;
                                    }
                            ...
                            // the face node index we are at should correspond to a leaf
                            // check the class of the element associated with the face
                            auto range = _face_to_elements_map.equal_range(_face_nodes[leaf_index].face);
                            if (range.first != range.second)
                            {
                                // hit
                                int found_elem_index = range.first->second;
...
void RefinedTetMesh::serialize(std::vector<std::byte>& buf) const
{
    TetMesh::serialize(buf);
    pack(buf, _element_refinement_level);
    pack(buf, _element_tree_nodes);
    pack(buf, _element_to_tree_node_map);
    pack(buf, _hanging_vertices);
    pack(buf, _edge_nodes);
    pack(buf, _face_nodes);
    pack(buf, _edge_to_edge_node_map);
    pack(buf, _face_to_face_node_map);
    pack(buf, _latest_new_vertices);
    pack(buf, _latest_removed_vertices);
    pack(buf, _latest_new_faces);
    pack(buf, _latest_new_elements);
    pack(buf, _latest_removed_elements);
    pack(buf, _latest_new_hanging_vertices);
    pack(buf, _latest_removed_hanging_vertices);
}

void RefinedTetMesh::deserialize(const std::byte*& buf)
{
    TetMesh::deserialize(buf);
    unpack(buf, _element_refinement_level);
    unpack(buf, _element_tree_nodes);
    unpack(buf, _element_to_tree_node_map);
    unpack(buf, _hanging_vertices);
    unpack(buf, _edge_nodes);
    unpack(buf, _face_nodes);
    unpack(buf, _edge_to_edge_node_map);
    unpack(buf, _face_to_face_node_map);
    unpack(buf, _latest_new_vertices);
    unpack(buf, _latest_removed_vertices);
    unpack(buf, _latest_new_faces);
    unpack(buf, _latest_new_elements);
    unpack(buf, _latest_removed_elements);
    unpack(buf, _latest_new_hanging_vertices);
    unpack(buf, _latest_removed_hanging_vertices);
}
