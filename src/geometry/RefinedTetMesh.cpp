     ...
     const Vec4i& base_element, RefinedElement& refined_element)
{
    ...
    else if ( p1_is_edge && p2_is_edge )
    {
        ...
        // F123
        else if (p1.edge == TetEdge::E12 && p2.edge == TetEdge::E13) new_vert.face = TetFace::F123;
        else if (p2.edge == TetEdge::E12 && p1.edge == TetEdge::E13) new_vert.face = TetFace::F123;

        else if (p1.edge == TetEdge::E12 && p2.edge == TetEdge::E23) new_vert.face = TetFace::F123;
        else if (p2.edge == TetEdge::E12 && p1.edge == TetEdge::E23) new_vert.face = TetFace::F123;

        else if (p1.edge == TetEdge::E13 && p2.edge == TetEdge::E23) new_vert.face = TetFace::F123;
        else if (p2.edge == TetEdge::E13 && p1.edge == TetEdge::E23) new_vert.face = TetFace::F123;

        // F124
        else if (p1.edge == TetEdge::E12 && p2.edge == TetEdge::E14) new_vert.face = TetFace::F124;
        else if (p2.edge == TetEdge::E12 && p1.edge == TetEdge::E14) new_vert.face = TetFace::F124;

        else if (p1.edge == TetEdge::E12 && p2.edge == TetEdge::E24) new_vert.face = TetFace::F124;
        else if (p2.edge == TetEdge::E12 && p1.edge == TetEdge::E24) new_vert.face = TetFace::F124;

        else if (p1.edge == TetEdge::E14 && p2.edge == TetEdge::E24) new_vert.face = TetFace::F124;
        else if (p2.edge == TetEdge::E14 && p1.edge == TetEdge::E24) new_vert.face = TetFace::F124;

        // F134
        else if (p1.edge == TetEdge::E13 && p2.edge == TetEdge::E14) new_vert.face = TetFace::F134;
        else if (p2.edge == TetEdge::E13 && p1.edge == TetEdge::E14) new_vert.face = TetFace::F134;

        else if (p1.edge == TetEdge::E13 && p2.edge == TetEdge::E34) new_vert.face = TetFace::F134;
        else if (p2.edge == TetEdge::E13 && p1.edge == TetEdge::E34) new_vert.face = TetFace::F134;

        else if (p1.edge == TetEdge::E14 && p2.edge == TetEdge::E34) new_vert.face = TetFace::F134;
        else if (p2.edge == TetEdge::E14 && p1.edge == TetEdge::E34) new_vert.face = TetFace::F134;

        // F234
        else if (p1.edge == TetEdge::E23 && p2.edge == TetEdge::E24) new_vert.face = TetFace::F234;
        else if (p2.edge == TetEdge::E23 && p1.edge == TetEdge::E24) new_vert.face = TetFace::F234;

        else if (p1.edge == TetEdge::E23 && p2.edge == TetEdge::E34) new_vert.face = TetFace::F234;
        else if (p2.edge == TetEdge::E23 && p1.edge == TetEdge::E34) new_vert.face = TetFace::F234;

        else if (p1.edge == TetEdge::E24 && p2.edge == TetEdge::E34) new_vert.face = TetFace::F234;
        else if (p2.edge == TetEdge::E24 && p1.edge == TetEdge::E34) new_vert.face = TetFace::F234;
    ...
    else if ( (p1_is_face && p2_is_edge) || (p2_is_face && p1_is_edge))
    {
        ...
        TetFace face = p1_is_face ? p1.face : p2.face;
        TetEdge edge = p1_is_edge ? p1.edge : p2.edge;

        // Case 4a: edge is adjacent to the face
        if ( (face == TetFace::F123 && (edge == TetEdge::E12 || edge == TetEdge::E13 || edge == TetEdge::E23)) ||
             (face == TetFace::F124 && (edge == TetEdge::E12 || edge == TetEdge::E14 || edge == TetEdge::E24)) ||
             (face == TetFace::F134 && (edge == TetEdge::E13 || edge == TetEdge::E14 || edge == TetEdge::E34)) ||
             (face == TetFace::F234 && (edge == TetEdge::E23 || edge == TetEdge::E24 || edge == TetEdge::E34)))
        {
            new_vert.face = face;
        }
    ...
    else if ( (p1_is_edge && p2.level == 0) || (p2_is_edge && p1.level == 0) )
    {
        ...
        if ( (edge == TetEdge::E12 && (vert == base_element[0] || vert == base_element[1])) ||
             (edge == TetEdge::E13 && (vert == base_element[0] || vert == base_element[2])) ||
             (edge == TetEdge::E14 && (vert == base_element[0] || vert == base_element[3])) ||
             (edge == TetEdge::E23 && (vert == base_element[1] || vert == base_element[2])) ||
             (edge == TetEdge::E24 && (vert == base_element[1] || vert == base_element[3])) ||
             (edge == TetEdge::E34 && (vert == base_element[2] || vert == base_element[3])) )
        {
            new_vert.edge = edge;
        }
    ...
    else if ( (p1.level == 0 && p2.level == 0) )
    {
        ...
        if      (p1.index == base_element[0] && p2.index == base_element[1])   new_vert.edge = TetEdge::E12;
        else if (p2.index == base_element[0] && p1.index == base_element[1])   new_vert.edge = TetEdge::E12;

        else if (p1.index == base_element[0] && p2.index == base_element[2])   new_vert.edge = TetEdge::E13;
        else if (p2.index == base_element[0] && p1.index == base_element[2])   new_vert.edge = TetEdge::E13;

        else if (p1.index == base_element[0] && p2.index == base_element[3])   new_vert.edge = TetEdge::E14;
        else if (p2.index == base_element[0] && p1.index == base_element[3])   new_vert.edge = TetEdge::E14;

        else if (p1.index == base_element[1] && p2.index == base_element[2])   new_vert.edge = TetEdge::E23;
        else if (p2.index == base_element[1] && p1.index == base_element[2])   new_vert.edge = TetEdge::E23;

        else if (p1.index == base_element[1] && p2.index == base_element[3])   new_vert.edge = TetEdge::E24;
        else if (p2.index == base_element[1] && p1.index == base_element[3])   new_vert.edge = TetEdge::E24;

        else if (p1.index == base_element[2] && p2.index == base_element[3])   new_vert.edge = TetEdge::E34;
        else if (p2.index == base_element[2] && p1.index == base_element[3])   new_vert.edge = TetEdge::E34;
    }
    else
    {
    ...
    if (new_vert.edge != TetEdge::NONE)
    {
        Edge edge;
        if (new_vert.edge == TetEdge::E12)  edge = Edge(base_element[0], base_element[1]);
        else if (new_vert.edge == TetEdge::E13)  edge = Edge(base_element[0], base_element[2]);
        else if (new_vert.edge == TetEdge::E14)  edge = Edge(base_element[0], base_element[3]);
        else if (new_vert.edge == TetEdge::E23)  edge = Edge(base_element[1], base_element[2]);
        else if (new_vert.edge == TetEdge::E24)  edge = Edge(base_element[1], base_element[3]);
        else if (new_vert.edge == TetEdge::E34)  edge = Edge(base_element[2], base_element[3]);
    ...
    else if (new_vert.face != TetFace::NONE)
    {
        Face face;
        if (new_vert.face == TetFace::F123) face = Face(base_element[0], base_element[1], base_element[2]);
        else if (new_vert.face == TetFace::F124) face = Face(base_element[0], base_element[1], base_element[3]);
        else if (new_vert.face == TetFace::F134) face = Face(base_element[0], base_element[2], base_element[3]);
        else if (new_vert.face == TetFace::F234) face = Face(base_element[1], base_element[2], base_element[3]);
...
void RefinedTetMesh::refineElement(int element_index, int refinement_level)
{
    ...
    const Vec4i& base_element = element(element_index);
    ...
    std::vector<Vec4i> parent_elements = {element(element_index)};
    ...
    for (int level = 0; level < refinement_level; level++)
    {
        ...
        for (const auto& parent_element : parent_elements)
        {
            ...
            if (level == refinement_level-1)
            {
                ...
                int e1 = _elements.push_back(std::move(elem1));
                int e2 = _elements.push_back(std::move(elem2));
                int e3 = _elements.push_back(std::move(elem3));
                int e4 = _elements.push_back(std::move(elem4));
                int e5 = _elements.push_back(std::move(elem5));
                int e6 = _elements.push_back(std::move(elem6));
                int e7 = _elements.push_back(std::move(elem7));
                int e8 = _elements.push_back(std::move(elem8));
