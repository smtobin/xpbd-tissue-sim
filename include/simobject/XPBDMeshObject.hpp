...
class XPBDMeshObject_<IsFirstOrder, SolverType, TypeList<ConstraintTypes...>> : public XPBDMeshObject_Base_<IsFirstOrder>
{
    ...
    using Base::_material_classes;
    using Base::_vertex_masses;
    using Base::_is_fixed_vertex;
    using Base::_vertex_applied_force;
    using Base::_sdf;
    using Base::_heat_solver;
    using Base::_adaptive_mesh_refinement;
    using Base::_max_refinement_level;
    using Base::_refinement_distance_threshold;
    ...
    addFaceOffsetAttachmentConstraint(int face_ind, const Vec3r& bary_coords, const Vec3r* attach_pos_ptr, const Vec3r& attachment_offset) override;

    /** Clears all element offset attachment constraint that are on this object. */
    virtual void clearFaceOffsetAttachmentConstraints() override;
